...
这是一个 Codex Skill。它不是一个完整的视频 SaaS，也不是随机素材拼接工具，而是一套可复用的视频生产流程：先研究真实 GitHub 项目，再抓取真实截图，生成结构化 brief，制作低文字密度的证据化讲解视频，并输出封面、字幕、预览图、QA 结果和平台发布文案。
...
- 旁白文案和旁白音频
- 玩梗标题、反差标题、干货标题三组候选，推荐标题优先选择有传播力但不虚假的表达
- 位于非字幕区域的小字说明：`纯干货分享，不存在站外引流`
...
- 标题和开头要有梗、有反差、有停留理由，但必须用真实截图、Star 数、文件或流程证据兜住
- 视频不起眼的非字幕区域必须放置小字：`纯干货分享，不存在站外引流`
